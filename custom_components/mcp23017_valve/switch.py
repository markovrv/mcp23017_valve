...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    if not os.path.exists(i2c_device):
        _LOGGER.error(f"I2C устройство {i2c_device} не найдено! Убедитесь, что I2C включён в системе.")
        return False

    try:
        # Тестовое подключение к MCP23017
        bus = smbus2.SMBus(1)
        bus.read_byte(DEFAULT_I2C_ADDRESS)  # Проверка связи
        bus.close()
    except Exception as e:
        _LOGGER.error(f"Ошибка доступа к I2C: {e}. Проверьте подключение MCP23017 и адрес {DEFAULT_I2C_ADDRESS}.")
        return False
...
class MCP23017Valve(SwitchEntity):
    def __init__(self, name, open_relay, close_relay, i2c_address, relay_open_time):
        self._name = name
